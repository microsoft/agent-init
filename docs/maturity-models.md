...
> This is the canonical reference for the maturity model and its assessment signals. For a more reflective companion on what the path feels like in practice — trust, habits, culture, and why Level 5 is hard — see [SDLC Readiness Assessment](maturity-in-practice.md).
...
| **PR template present**                             | .github/PULL_REQUEST_TEMPLATE.md                                                              | Structures every PR so human and agent authors provide linked issue, test plan, and review context consistently                      | Agent-generated PRs arrive under-explained, forcing reviewers to reconstruct context before they can assess the code                                                                  |
...
| **Branch protection configured**       | .github/rulesets/\*.json or branch protection config    | Required status checks and merge protections keep autonomous PRs from bypassing review or CI guardrails                   | Agents can merge or ship changes without the same protections human-authored work is expected to pass through                          |
| **Commit message convention enforced** | commitlint/commitizen config or dependency              | Structured commit history enables automated changelogs, release tooling, and reliable downstream workflow automation      | Releases and changelogs stay manual, and agent-authored commits drift into inconsistent formats that are hard to process automatically |
| **PR auto-labeling configured**        | .github/labeler.\* or actions/labeler workflow          | Auto-labeling routes agent-authored PRs to the right reviewers and workflows without manual triage                        | Autonomous PRs stall waiting for humans to triage or manually attach the metadata downstream automation depends on                     |
...
> **Note on checks:** Level 4 now includes an explicit structural signal: release automation. Reaching this level still requires more than file presence alone — all Level 1–3 checks must pass, and the MCP, agent, and skill infrastructure validated at lower levels must be actively exercised in real workflows. The remaining gap is still operational, but release automation makes that workflow maturity easier to verify.
...
- Release automation is configured so versioning and changelog steps do not require manual handoffs
