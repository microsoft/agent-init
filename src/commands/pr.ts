...
export async function prCommand(repo: string | undefined, options: PrOptions): Promise<void> {
  ...
  if (provider === "azure") {
    ...
    const repoPath = path.join(cacheRoot, organization, project, name);
    ...
    const instructions = await generateCopilotInstructions({ repoPath, model: "gpt-4.1" });
...
function buildInstructionsPrBody(): string {
  return [
